 ...
 *     {
 *       'baseUrl':
 *           The URL base path to the CAS server.
 *       'loginService':
 *           (optional) The path portion of the CAS login URL. Will be
 *           resolved against the baseURL.
 *           Default: login
 *       'validationService':
 *           (optional) The path portion of the CAS ticket validation URL. Will be 
 *           resolved against the baseURL.
 *           Provide this param only if you have a custom path, otherwise, it will
...
var CAS = function CAS(options) {
    ...
    var cas_url = url.parse(options.baseUrl);
    if (cas_url.protocol != 'https:') {
        throw new Error('Only https CAS servers are supported.');
    }
    if (!cas_url.hostname) {
        throw new Error('Option `baseUrl` must be a valid url like: https://example.com/cas');
    }
    ...
    this.loginService = options.loginService || 'login';
...
CAS.prototype.authenticate = function (req, res, callback, service) {
    var casAuthenticateURL = new URL(this.loginService, 'https://' + this.hostname + ':' + this.port + this.base_path);
    ...
    if (!service) {
        ...
        if (this.service) {
            console.log('this.service === true')
            service = new URL(service);
            reqURL.searchParams.forEach(function (value, name) {
                service.searchParams.set(name, value)
            })
        } else {
            service = url.format({
        ...
        service = new URL(service);
        console.log(reqURL)
        if (reqURL.search && reqURL.search.length > 1) {
            // Copy request URL search params to the cas service URL
    ...
    if (!ticket) {
        ...
        casAuthenticateURL.searchParams.set('service', service.href);
        res.writeHead(307, { 'Location': casAuthenticateURL.href });
        res.write('<a href="' + casAuthenticateURL.href + '">CAS login</a>');
        res.end();
    }
...
CAS.prototype.validate = function (ticket, callback, service, renew) {
    ...
    var validateUri = this.validationService;
        ...
        res.on('end', function () {
            ...
            // CAS 2.0 (XML response, and extended attributes)
                ...
                if (elemSuccess && elemSuccess.length > 0) {
                    ...
                    var elemGroups = $('cas\\:groupes').first();
                    var groups = elemGroups.text().split('|');
